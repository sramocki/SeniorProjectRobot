...
def main():
    ...
    # if there is no path supplied, then grab reference to camera
    ...
    while True:
        ...
        # resize the video (this is harcoded)
        frame = cv2.resize(frame, (400, 200))

        # check to see if the ROI has been computed
        ...
        # handle if the 'i' key is pressed, then go 
        # into ROI selection mode
        if key == ord("i") and len(roiPts) < 4:
            # indicate that we are in input mode and clone the frame
            inputMode = True
            orig = frame.copy()

            # keep looping until all ROI points have
            # been selected; press any key to exit ROI selection
            # mode once points are selected
            while len(roiPts) < 4:
                cv2.imshow("frame", frame)
                cv2.waitKey(0)
